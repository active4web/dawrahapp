			...
			<div class="page-content-wrapper">
				...
				<div class="page-content">
					...
					<ul class="page-breadcrumb breadcrumb">
						...
						<li>
							<span class="active">الحقائب </span>
						</li>
					</ul>
					<!-- END PAGE BREADCRUMB -->
							...
							<div class="portlet light bordered">
								<div class="portlet-title">
									<div class="caption font-dark">
										...
										<span class="caption-subject bold uppercase">الحقائب </span>
									</div>
										<!--<div class="caption font-dark" style="float:left">
<form action="#" class="ssform" autocomplete="off">
<input type="text" name="name" placeholder="البحث بالاسم" id="username_search">
<ul class="ss"></ul>
</form>
</div>-->
								</div>
									...
									<table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1_2">
										<thead>
											<tr>
												...
												<th>الاسم </th>
												<th>اجمالى الأيام</th>
												<th>عدد الأيام فى الاسبوع</th>
												<th> الساعات </th>
												<th> اجمالى التقييم </th>
												<th> عدد التقييمات  </th>
												<th> المسئول   </th>
												<th> المحتوى   </th>
												<th>الحالة </th>
							...
							$intersted_courses=$this->db->get_where("reviews",array("id_course"=>$data->id,'course_key'=>2))->result();
							$user_name = get_table_filed('customers',array('id'=>$user_id),"user_name");
                                        ?>
											<tr class="odd gradeX">
												...
												<td> <?=mb_substr($data->bage_name,0,50);?> </td>
												<td><?=mb_substr($data->bage_total_daies,0,50);?></td>
												<td> <?=mb_substr($data->week_bage_daies,0,50);?></td>
												<td> <?=mb_substr($data->bage_hrs,0,50);?> </td>
												
                                                 <td> <?=$data->total_rate;?> </td>
												<td> <a href="<?=$url?>admin/courses/rate_details?id=<?=$data->id;?>&course_type=2"><?= count($intersted_courses);?></a></td>
												<?php if($user_id!=0){?>
												<td> <a href="<?=$url?>admin/users/provider_details?id=<?=$data->id;?>"> <?=mb_substr($user_name,0,50);?> </td>
												<?php } else {?>
													<td> الأدارة </td><?php }?>
												<td><a href="<?= $url?>admin/courses/bags_content?course_id=<?= $data->id?>">المحتوى</a></td>
													<td> 
												...
												<td>
													<div class="btn-group">
														...
														<ul class="dropdown-menu pull-left" role="menu">
															...
															<li><a href="<?=$url?>admin/courses/bages_details?id=<?=$data->id;?>&course_type=2"><i class="fa fa-eye"></i> تفاصيل </a></li>
														<li><a href="<?=$url?>admin/courses/edit_bag?id=<?=$data->id;?>"><i class="fa fa-pencil"></i> تعديل </a></li>
...
$(document).ready(function(e) {
	$(".delbutton").click(function(e) {
	 ...
	 $(".addbutton").click(function(e) {
        window.location.assign("<?= DIR?>admin/courses/add_bag");
    });
});
</script>
...
var data={id:id};
			$.ajax({
				url: '<?php echo base_url("admin/courses/active_bag") ?>',
                type: 'POST',
                data: data,				
	...
	$(".confirm_delete").click(function(e) {
     ...
     var b=confirm("حذف الدورة قد تؤدى  الىى مشاكل فى عرض التفاصيل العامة ,هل انت متاكد من عملية الحذف");
    if(b==true){
      window.location.assign("<?=$url?>admin/courses/delete_bags?id_customers="+iduser);  
    }
